...
class EcomPage {
  ...
  constructor(page: Page) {
    ...
    this.submitbtn =
      page.locator("input#login");

    this.elements =
      page.locator("//div[@class='card-body']");
    ...
    this.cartItem =
      page.locator("div.cartSection h3");

    this.checkoutbth =
      page.locator("//button[text()='Checkout']");
    ...
    this.order_id =
      page.locator("tr.ng-star-inserted th");
  }
  ...
  async addToCart(
    ...
    await expect(this.cartItem)
      .toHaveText(itemtoadd);
  }
  ...
  async performCheckout(): Promise<void> {
    ...
    await this.nameFiled.fill(
      "Onkar Deepakrao Pethe"
    );

    await this.countryFiled
      .pressSequentially("Ind");

    await this.countryOpt.click();
    ...
    const purchaseid =
      await this.purchaseidb.textContent();
    ...
    const ele_count: number =
      await this.order_id.count();
